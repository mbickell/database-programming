...
def get_total_entries(cursor, propertyID):
    execute(
        f"SELECT COUNT(id) FROM electricity WHERE property={propertyID}", limit=True)
...
def get_average_value(cursor, propertyID):
    execute(
        f"SELECT AVG(value) FROM electricity WHERE property={propertyID}", limit=True)
...
def get_minimum_value(cursor, propertyID):
    execute(
        f"SELECT value, timestamp FROM electricity WHERE value = (SELECT min(value) FROM electricity WHERE property={propertyID})", limit=True)
...
def get_group_by_minimum_value(cursor):
    execute(group_by_query_builder(
        "electricity.timestamp, MIN(electricity.value) as minimum"))
    ...
    while i < len(data):
        execute(
            f"SELECT timestamp FROM electricity WHERE value = {data[i][3]}", limit=True)
...
def get_maximum_value(cursor, propertyID):
    execute(
        f"SELECT value, timestamp FROM electricity WHERE value = (SELECT max(value) FROM electricity WHERE property={propertyID})", limit=True)
...
def get_group_by_maximum_value(cursor):
    execute(group_by_query_builder(
        "electricity.timestamp, MAX(electricity.value) as maximum"))
